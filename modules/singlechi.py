...
# Number of fit parameters consumed by each circuit element. Single source of
# truth shared by flatten_params (validation) and format_circuit_output
# (labeling) so the two can never desync again.
ELEMENT_PARAM_COUNT = {
    'R': 1, 'C': 1, 'L': 1, 'W': 1,
    'Q': 2, 'F': 2, 'G': 2,
    'H': 3,
...
def _elem_param_labels(elem_type, num):
    """Return the parameter label(s) for a single circuit element.

    The labels for R, C, L and Q are kept byte-identical to the previous
    implementation so existing reports do not change. W, F, G and H were
    previously unhandled (which silently misaligned every parameter printed
    after them); they are now labeled explicitly.
    """
    if elem_type in ('R', 'C', 'L', 'W'):
        return [f"{elem_type}{num}"]
    if elem_type == 'Q':
        return [f"Q{num}", f"n{num}"]
    if elem_type == 'F':
        return [f"F{num}", f"F{num}_n"]
    if elem_type == 'G':
        return [f"G{num}", f"t_G{num}"]
    if elem_type == 'H':
        return [f"H{num}", f"t_H{num}", f"phi_H{num}"]
    raise ValueError(f"Unsupported element: {elem_type}{num}")
...
def flatten_params(params, circuit_str=None, N_sub=None):
    ...
    if circuit_str and N_sub:
        # Parse circuit elements including L, W, F, G and H
        elements = re.findall(r'([RQCLWFGH])(\d+)', circuit_str)
        param_idx = 0

        for elem_type, elem_num in elements:
            count = ELEMENT_PARAM_COUNT.get(elem_type)
            if count is None:
                raise ValueError(f"Unsupported element: {elem_type}{elem_num}")
            # Validate every parameter slot this element consumes.
            for k in range(count):
                validate_param(params[param_idx + k], elem_type, elem_num)
            param_idx += count
...
def format_circuit_output(popt, perror, CorrM, circuit_str, N_sub, Temp, param_template):
    ...
    N_sub : int
        Number of sublists
    Temp : array-like
        Temperature values (used to label per-sublist/local parameters)
    param_template : list
    ...
    # Uniform, table-driven expansion. Every element type (R, C, L, W, Q, F, G, H)
    # is handled the same way, so the per-element index can no longer desync.
    for elem_type, elem_num in elements:
        for base in _elem_param_labels(elem_type, elem_num):
            if template_idx >= len(param_template):
                break

            try:
                if param_template[template_idx]:
                    # Per-sublist (local / temperature-dependent) parameter
                    for i in range(N_sub):
                        param_labels.append(f"{base}_{Temp[i]}°C")
                        param_values.append(popt[flat_idx + i])
                        param_errors.append(perror[flat_idx + i])
                    flat_idx += N_sub
                else:
                    # Global (temperature-independent) parameter
                    param_labels.append(base)
...
def Single_chi_report(f, Z, params, Temp, circuit_str, weight_mtd='M'):
    ...
    -------
    tuple
        (popt, perror, Z_fit) - Optimized parameters, errors and fitted impedance.
    """
    report_content = []
