...
def format_circuit_output(popt, perror, CorrM, circuit_str, N_sub, param_template):
    """
        ...
        try:
            if elem_type in ['R', 'L', 'C']:
                if param_template[template_idx]:
                    # Per-sublist parameter
                    for i in range(N_sub):
                        param_labels.append(f"{elem_type}{elem_num},{i}")
                ...
                if param_template[template_idx]:
                    for i in range(N_sub):
                        param_labels.append(f"Q{elem_num},{i}")
                        param_values.append(popt[flat_idx + i])
                        param_errors.append(perror[flat_idx + i])
                    flat_idx += N_sub
                else:
                ...
                if template_idx < len(param_template):
                    if param_template[template_idx]:
                        for i in range(N_sub):
                            param_labels.append(f"n{elem_num},{i}")
                            param_values.append(popt[flat_idx + i])
                            param_errors.append(perror[flat_idx + i])
                        flat_idx += N_sub
                    else:
...
def Single_chi_report(f, Z, params, Temp,circuit_str):
    """
    ...
    # Run EISFit directly
    chi2, R_chi2, AICc, popt, perror, R_squared, CorrM, pcov = fit.EISFit(
        f, Z, flat_params, circuit_callable, 
        UB=None, LB=None, 
        weight_mtd='M', method='lm',
        single_chi='Yes'
    )
    
    # Format output with circuit awareness
    param_df, corr_df = format_circuit_output(np.abs(popt), np.abs(perror), CorrM, circuit_str, N_sub, param_template)
    
    # Print report
    report = []
    report.append("{:=^72s}".format(" EIS Fit Reports "))
    report.append(f"\nCircuit: {circuit_str}")
    report.append(f"Chi-square: {chi2:.2e}")
    report.append(f"R-squared: {R_squared:.4f}")
    report.append("\nFitted Parameters:")
    report.append("==================")
    report.append(param_df.to_string(index=False))
    report.append("\nCorrelation Matrix:")
    report.append("==================")
    report.append(corr_df.to_string())
    report.append("\n{:=^72s}".format(" EIS Fit plots "))
    print("\n".join(report))
    
    # Calculate fitted impedance for plotting
    ...
    return popt, perror, Z_fit, figN, figB
...
def generate_plots(f, Z, Z_fit, Temp):
    ...
    for i, (fi, Zi, Zf, marker, label) in enumerate(zip(f_s, Z_data_s, Z_fit_s, markers, labels)):
        figN, ax = plt.subplots(figsize=(5, 5), dpi=150)
        # Data points
        ...
        # Save Nyquist plot
        plt.savefig(f'{mypath}/s-chi_nyquist_plot_{label}.svg', bbox_inches='tight', dpi=300)
    ...
    # Save Bode plot
    plt.savefig(f'{mypath}/s-chi_stack_bode_plot.svg', bbox_inches='tight', dpi=150)
